...
<img src="images/icons/cache.png" width="22" alt=""> beside the meter says what your next turn is about to cost.

Your conversation so far is a cached prefix. While it holds, resending it runs about a tenth of input price.

- **Nothing.** The cache holds. The ordinary state.
- **Amber** through the last quarter of the TTL, brightening as it closes. Send now and you still read from cache.
- **Red, bold.** Cold. The next turn rewrites the whole prefix at the cache-write premium.

Only an idle session reaches amber, since an active one keeps refreshing, so it turns up while there's still time to
act. Nothing shows till the first response reports cache tokens.

The glyph links to [Anthropic's prompt-caching docs](https://platform.claude.com/docs/en/build-with-claude/prompt-caching).
`CACHE_WARN_AT` moves the amber window (25), `CACHE_HELD_ICON` marks the holding state too, and `CACHE_URL=` drops
the link.
