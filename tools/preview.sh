...
# Just the three lines and the footer. The title block is composited on top afterwards.
rows = 10
...
put(4,  lines[1])
put(6,  lines[2])
put(8,  f"{DIM}One bash file. No dependencies.{OFF}")
...
PY
  ...
  ffmpeg -y -v error -i "$WORK/social.gif" -frames:v 1 "$WORK/social-lines.png" || return 1

  # The title is drawn here rather than rendered as terminal text, because a terminal cannot glow. Same font as the
  # line below it, so the card still reads as one thing.
  CARD_LINES="$WORK/social-lines.png" CARD_THEME="$WORK/theme.json" CARD_OUT="$out" \
  CARD_FONT="$(python3 -c 'import json,sys;print(json.load(sys.stdin).get("font","").split(",")[0])' < "$WORK/theme.json")" \
  python3 - <<'SOCIALPY' || return 1
import json, os, sys
from pathlib import Path
...
W, H = 1280, 640
theme = json.load(open(os.environ["CARD_THEME"]))
bg = theme.get("bg", "#232523").lstrip("#")
BG = tuple(int(bg[i:i+2], 16) for i in (0, 2, 4))
TITLE, TAG = "#5FC9F8", "#C8CCCE"

# The upright regular, explicitly. Matching on the family name alone picks up Italic and Bold faces, and a slanted
# title next to an upright status line looks like a mistake.
fam = os.environ.get("CARD_FONT") or "Hack Nerd Font Mono"
key = fam.replace(" ", "").lower()
path = None
for d in (Path.home() / "Library/Fonts", Path("/Library/Fonts")):
    for f in sorted(d.glob("*.ttf")) if d.is_dir() else []:
    ...
    if path:
        break
if not path:
    sys.exit("no upright regular face for %r" % fam)

card = Image.new("RGB", (W, H), BG)
title_f = ImageFont.truetype(str(path), 78)
tag_f = ImageFont.truetype(str(path), 25)

# Glow: the same text blurred underneath itself, twice, so the halo falls off rather than ringing.
glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
ImageDraw.Draw(glow).text((60, 96), "claude-dipstick", font=title_f, fill=TITLE)
for radius, alpha in ((34, 210), (16, 235), (6, 255)):
...
d.text((60, 96), "claude-dipstick", font=title_f, fill=TITLE)
d.text((62, 214), "Context, model, session usage, week usage, budget/cost and more.", font=tag_f, fill=TAG)

lines = Image.open(os.environ["CARD_LINES"]).convert("RGB")
scale = min((W - 96) / lines.width, 1.0)
lines = lines.resize((round(lines.width * scale), round(lines.height * scale)), Image.LANCZOS)
card.paste(lines, ((W - lines.width) // 2, 286))
card.save(os.environ["CARD_OUT"])
SOCIALPY
