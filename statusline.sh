...
# The flame is the one mark here that names a billing mechanism rather than a number you already understand, so it
# links to the page explaining it. Blank CACHE_URL to drop the link and keep the glyph.
...
cache_url="${CACHE_URL-https://platform.claude.com/docs/en/build-with-claude/prompt-caching}"
...
if [ "$show_cache" = "1" ] && [ "$payload_cache_observed" = "true" ]; then
  ...
  # Linked on every tier it draws, held included. The glyph is the whole element, so there is no name to carry the
  # link the way the branch and profile do, and a mark you cannot look up is a worse mark.
  if [ -n "$cache_glyph" ]; then
    link "$cache_url" "$cache_glyph"
    cache_text="${cache_weight}${cache_fg}${linked}${RESET}"
  fi
fi
